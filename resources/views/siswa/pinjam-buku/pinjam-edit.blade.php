...
<head>
    ...
    <title>Edit Peminjaman | SIPUS SMEKDA</title>
    ...
    <div class="form-wrapper">
        <div class="glass-card">
            <header class="card-header-luxe">
                <h2><i class="bi bi-pencil-square me-2"></i>Edit Pengajuan</h2>
                <p style="color: var(--teks-muted); font-size: 0.85rem; margin: 5px 0 0 5px;">Perbarui data peminjaman buku Anda</p>
            </header>

            <div class="card-body-luxe">
                <form action="{{ route('pinjam.update', $transaksi->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    
                    <div class="section-tag">1. Identitas</div>
                    <div class="field-group">
                        <label>Nama Anggota</label>
                        <input type="text" class="custom-input readonly-mode" value="{{ auth()->user()->nama }}" readonly>
                    </div>

                    <div class="section-tag">2. Pilih Koleksi</div>
                    <div class="search-container">
                        <i class="bi bi-search"></i>
                        <input type="text" id="searchBuku" class="custom-input" placeholder="Cari buku lain...">
                    </div>

                    <div class="book-grid" id="bookGrid">
                        @foreach($bukus as $b)
                        <div class="book-item {{ $transaksi->buku_id == $b->id ? 'selected' : '' }}" 
                             data-id="{{ $b->id }}" 
                            ...
                            <img src="{{ $b->cover ? asset('storage/' . $pathCover) : 'https://via.placeholder.com/150x200?text=SMEKDA' }}" 
                    ...
                    <input type="hidden" name="buku_id" id="selected_buku_id" value="{{ old('buku_id', $transaksi->buku_id) }}">
                    @error('buku_id')
                        <span class="error-msg"><i class="bi bi-exclamation-circle me-1"></i>{{ $message }}</span>
                    @enderror

                    <div class="section-tag">3. Detail Peminjaman</div>
                    <div class="row g-3">
                        <div class="col-md-6 field-group">
                            <label>Tanggal Pinjam</label>
                            <input type="date" name="tgl_pinjam" id="tgl_pinjam" 
                                   class="custom-input @error('tgl_pinjam') is-invalid @enderror" 
                                   value="{{ old('tgl_pinjam', $transaksi->tanggal_pinjam->format('Y-m-d')) }}" 
                    ...
                    <a href="{{ route('pinjam') }}" class="btn-batal">Batalkan</a>
                </form>
            </div>
        </div>
    </div>
